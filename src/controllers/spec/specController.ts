...
const specController = {
    ...
    show : async  ( req: AuthRequest, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            if(!model) return res.status(404).json({status: 404, message : 'Model not found'})
    ...
    edit : async (req : Request, res : Response, next : NextFunction) => {
        try{
            ...
            return res.status(200).json({message:'Model Updated Successfully', status : 200, data : model} as ResponseBody)
        }catch(error) {
